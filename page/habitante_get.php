   ...
   /* 1. Verificar si ya hay una sesión activa para continuar con el proceso*/  
     ...
     if (is_countable($datos)) {   
        ...
        if ($datos['rol_id'] == 2 || $datos['rol_id'] == 1) {    
            ...
            if (isset($_POST['habitante_id'])) {
            
                $id_jefe = ($datos['rol_id'] == 2)?$_SESSION['user']:$_GET['id_jefe'];

                $habitante_id = $_POST['habitante_id'];
            
                $resultado = $conexion->prepare("SELECT habitante_id, nombre, apellido, ci_ps_pn, fecha_nacimiento, edad, genero, telefono, vivienda_id, grado_instruccion FROM habitante WHERE md5(habitante_id) =?");
                $resultado->execute([$habitante_id]);
                $rows = $resultado->rowcount();
                $habitante = [];
            
                if ($rows > 0){
                    $habitante = $resultado->fetch(PDO::FETCH_ASSOC);
                   // var_dump($vivienda);
                }
            
                $habitante["habitante_id"] = md5($habitante["habitante_id"]); 
