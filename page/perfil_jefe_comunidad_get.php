   ...
   /* 1. Verificar si ya hay una sesión activa para continuar con el proceso*/  
     ...
     if (is_countable($datos)) {   
        ...
        if ($datos['rol_id'] == 1) {            
            ...
            if (isset($_POST['jefe_comunidad_id'])) {

                $jefe_comunidad_id = $_POST['jefe_comunidad_id'];
            
                $resultado = $conexion->prepare('SELECT j.jefe_comunidad_id, j.usuario_id, j.ci_ps, j.nombre, j.apellido, j.correo, u.nombre_usuario FROM jefe_comunidad as j
                INNER JOIN usuario as u
                ON u.usuario_id=j.usuario_id
                WHERE j.estado=? AND u.estado=? AND md5(jefe_comunidad_id)=?');
                $resultado->execute(["1","1",$jefe_comunidad_id]);
                $rows = $resultado->rowcount();
                $jefe_comunidad = [];
            
                if ($rows > 0){
                    $jefe_comunidad = $resultado->fetch(PDO::FETCH_ASSOC);
                   // var_dump($jefe_calle);
                }
                
                $jefe_comunidad["jefe_comunidad_id"] = md5($jefe_comunidad["jefe_comunidad_id"]);
                $jefe_comunidad["usuario_id"] = md5($jefe_comunidad["usuario_id"]);
                echo json_encode($jefe_comunidad);
                }
