  ...
  /* 1. Verificar si ya hay una sesión activa */  
    ...
    if (is_countable($datos)) {   
       ...
       if ($datos['rol_id'] == 1) {
            ...
            $datos_sesion = $conexion->prepare('SELECT j.jefe_comunidad_id, j.usuario_id, j.nombre, j.apellido, j.ci_ps, j.correo, u.nombre_usuario FROM jefe_comunidad AS j
          ...
          if (isset($_POST['EditarPerfil'])){
            ...
            $jefe_comunidad_id2 = $_POST['jefe_comunidad_id'];
            ...
            if (empty($error_nombre_editar_1) and empty($error_nombre_editar_2) and empty($error_apellido_editar_1) and empty($error_apellido_editar_2) 
                ...
                and empty($error_user_editar_1) and empty($error_user_editar_2)){
                    ...
                    if ($rows_editar > 0) {
                        ...
                        /* 4. Actualizamos los datos de Jefe de Comunidad */
                        $actualizar_jefe_comunidad = $conexion->prepare("UPDATE jefe_comunidad SET ci_ps = ?, nombre = ?, apellido = ?, correo = ?, fecha_actualizacion = ?  WHERE md5(jefe_comunidad_id) = ?;");
                        $actualizar_jefe_comunidad->execute([$ci_ps_editar,$nombre_editar,$apellido_editar,$correo_editar,$fecha_actualizacion,$jefe_comunidad_id2]);
...
<head>
    ...
    <!-- CSS Estilo -->
    <link rel="stylesheet" href="../src/css/perfil-jefe-comunidad.css">
    ...
    <div class="row g-0 row-cols-12">
        ...
        <div class="container col-10  bajar">
            ...
            <div class="contenido col-7 mt-5 d-flex justify-content-center">
                <button class="px-3 py-2 btn boton_perfil" data-bs-toggle="modal" data-bs-target="#modalEditarPerfil" data-bs-id="<?= md5($d['jefe_comunidad_id']); ?>">Editar perfil</button>
            ...
            <div class="modal-dialog modal-md">
                <div class="modal-content">
                  ...
                  <div class="modal-body">
                    <form action="perfil_jefe_comunidad.php" method="POST">
                        <div class="modal-content border-white">

                            <img src="../src/img/mi_perfil.png" class="w-25 mb-4 modal_imagen" alt="">
                            
                            <input type="hidden" id="jefe_comunidad_id" name="jefe_comunidad_id" >
...
/* ---------------------------------------------------- EDITAR REGISTRO ------------------------------------------------------------------------- */
    ...
    modalEditarPerfil.addEventListener('show.bs.modal', event => {         //le añadimos el evento que permita abrir la ventana modal gracias a shown.bs.modal
        ...
        let jefe_comunidad_id = button.getAttribute('data-bs-id')                      //el atributo "data-bs-id" debe estar incluido en el boton para poder pasarle el "manzana_id" del registro que quiero modificar

        let inputJefeComunidadId = modalEditarPerfil.querySelector('.modal-body #jefe_comunidad_id')              //selecionamos la clase y el id presentes en el formulario
        ...
        let url = "perfil_jefe_comunidad_get.php"                 //definimos la ruta donde vamos a realizar la petición
        let formData = new FormData()
        formData.append('jefe_comunidad_id', jefe_comunidad_id)
        ...
        .then(data => {                           //la variable data contendra todos los datos del registro
            inputJefeComunidadId.value = data.jefe_comunidad_id      //lo que viene a continuacion de data. es el campo al cual se llama en la consulta a la BD
